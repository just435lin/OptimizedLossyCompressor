...
int main(int argc, char* argv[])
{
    ...
    float compressionRatioAvg = 0;
    float compressionThroughputAvg = 0;
    float decompressionThroughputAvg = 0;
    int counter = 0;

    char dirname[256] = "./1800x3600";
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        size_t cmpSize = 0;
        int status=0;
        oriData = readFloatData_Yafan(srcFilePath, &nbEle, &status); //data
        ...
        float errorBound1 = errorBound * (max_val - min_val);

        // Compression.
        float *compressResults = hawkZip_compress_Test(oriData, cmpData, nbEle, &cmpSize, errorBound1);
        compressionRatioAvg += compressResults[0];
        compressionThroughputAvg += compressResults[1];
        free(compressResults);
        // Decompression.
        float decompressResult = hawkZip_decompress_Test(decData, cmpData, nbEle, errorBound1);
        decompressionThroughputAvg += decompressResult;
        ...
        for(size_t i=0; i<nbEle; i++)
        {
            ...
            if(fabs(oriData[i]-decData[i])>errorBound1*1.1) 
            {
    ...
    compressionRatioAvg = compressionRatioAvg/counter;
    compressionThroughputAvg = compressionThroughputAvg/counter;
    decompressionThroughputAvg = decompressionThroughputAvg/counter;

    printf("hawkZip   average compression ratio:      %f\n", compressionRatioAvg);
    printf("hawkZip   average compression throughput: %f GB/s\n", compressionThroughputAvg);
    printf("hawkZip   average decompression throughput: %f GB/s\n", decompressionThroughputAvg);
